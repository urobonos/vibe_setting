...
name: debate
description: >
  Multi-Agent Team Debate Protocol. `/taskflow:debate` 슬래시로 **명시 호출** 시에만 4 에이전트팀 (각 팀 4 Agent = 총 16 Agent) 풀-병렬 spawn 토론을 수행한다 (토큰 4× 비용 — 자동 발동 없는 수동 호출 전용).
...
**수동 호출 전용** — `/taskflow:debate` 슬래시로만 진입한다. 자연어 자동 발동 없음 (16 Agent spawn·토큰 4× 비용 보호). 토론이 필요해 보여도 Lead 는 `/taskflow:debate` 호출을 *안내*만 하고 자동 spawn 하지 않는다.
...
**Model:** (orchestration §1.1 정합)
- Lead 4개 + 멤버 12개 = `opus` 통일 (기본값 — Agent 도구 `model` 생략 시 부모 opus 상속)
- 경량 질문 (1~2 문장) 멤버는 `sonnet` 하향 가능 (Lead 재량, 강제 아님)
- effort(Max/High) 파라미터는 Agent 도구에 없어 미표기 (effort 차등은 Workflow `agent()` 경로만)
...
| 2026-05-15 | 2.2.0 | `/taskflow:debate` trigger 추가 |
