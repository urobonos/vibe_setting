...
**적대적 검증 게이트는 여기서 돌리지 않는다 (2026-08-31~).** `taskflow:adversary` 는 **클린을 만든 그 루프의 종료 게이트**라 생산 경로(`code`·`tick`·`review`)에만 붙는다 (`code.md` §"적대적 검증 게이트"). watch 는 이미 게이트를 통과한 산출물을 **다른 리뷰어 편차로 재확인**하는 자리이므로, 여기서 또 깨보게 하면 같은 코드에 red-team 이 두 번 붙는다 — tick 이 통과시킨 것을 watch 가 다른 기준으로 반려하던 그 왕복을 재생산한다.
...
- 2026-09-10: **§"코드 축" 리뷰어 입력에 `BASELINE` 동봉을 추가** (무인 코드리뷰 계약 SSOT 이므로 `tick` 도 상속). 없으면 리뷰어가 덮는 명령을 스스로 찾고, 그 탐색이 전량 스위트로 넓히려는 유인이 된다 — 이 환경에서 그건 실 테이블을 갈아엎는다. 부재 시엔 **부재를 명시**한다(조용히 비우지 않는다). 배선 SSOT = `code.md` §"기준선을 교차 확인한다"
- 2026-08-31: **적대적 검증 게이트(`taskflow:adversary`)는 watch 에서 돌리지 않음을 명시.** 게이트는 클린을 만든 루프의 종료 게이트라 생산 경로(`code`·`tick`·`review`)에만 붙는다 — 재확인 자리에 red-team 을 또 붙이면 같은 코드에 두 번 붙고, 없애려던 왕복이 되살아난다 (배선 SSOT = `code.md`)
