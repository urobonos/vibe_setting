...
> **5축이 `cold-reviewer` §"1. 기능 오류" 와 1:1 이다** (경계값·예외흐름·타입계약·회귀·동시성). 리뷰어가 보는 축을 DoD 가 안 받으면 그 축은 매번 반려로만 발견된다.
...
그래서 지시문이 아니라 **DoD** 로 넣는다. DoD 는 `cold-reviewer` 의 대조 기준이라 항목별 확인 지점이 생긴다.
...
`step-developer` 는 `TESTS: {실행한 검증과 결과}` 를 반환하고 `cold-reviewer` 는 §"6. 돌려봐야 검증" 에서 그 테스트가 **그 변경을 실제로 덮는지**를 본다. 계획이 명령을 안 주면 대조할 기준이 없어 리뷰어는 6축에서 지적을 낼 수밖에 없다 — 개발이 뭘 돌렸든 그게 맞는 검증이었는지 판정이 불가능하기 때문이다.
...
- [ ] **검증:** 각 step DoD 마지막 줄이 **실행 명령**인가 — "동작 확인"·"정상 반환" 같은 문장은 FAIL (`cold-reviewer` 6축이 대조할 기준이 없다)
