...
### 병렬로 여러 개 돌리고 싶다면 — `tick-team` 을 쓴다

tick 은 `registry_claim`(lock 안 원자)으로 배타 점유하므로 **여러 인스턴스가 동시에 돌아도 같은 step 을 중복 작업하지는 않는다.** 그러니 병렬 자체는 논리적으로 안전하다. 그런데도 이 커맨드가 1개만 허용하는 이유는, 병렬이 필요한 상황의 정답이 이미 `/taskflow:tick-team` 이기 때문이다 — 동적 워커 풀 + 자율 claim + `isolation: worktree` 격리를 갖췄다. tick-loop 을 N 개 띄우는 것은 그것을 조잡하게 재구현하는 셈이고, 진행 가능 step 보다 많은 인스턴스는 전부 "진행 가능 없음" 으로 헛돌며 매번 시작 컨텍스트만 태운다.

**여러 세션에 걸쳐 띄우는 것도 같다.** PID 파일이 `state/` 하나라 다른 세션에서도 중복 기동은 거부된다.

바깥(프로세스 리셋)과 안(워커별 리셋)을 겹치려면 tick 대신 tick-team 을 루프에 넣는다 — 단 `tick-team.md` 가 "단일 tick 안정화 후 사용" 을 권고하므로 순서를 지킨다.
