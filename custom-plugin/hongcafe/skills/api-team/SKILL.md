...
**Why:** 조회계는 prod 영향 0 + 분석 정확성에 직결되므로 즉시 실행이 최선이나, 변경계는 cold context 인 INF 멤버가 사용자 승인 없이 prod Lambda env·SSM·IAM 을 바꾸면 즉시 운영 장애로 번지고 멤버 컨텍스트 종료와 함께 롤백 경로도 사라진다. 권고 ↔ 실행 분리는 영향 격리의 최후 방어선.
...
**Why:** 3-멤버 결과를 raw 그대로 concat 하면 사용자가 직접 충돌·누락을 찾아내야 해서 3-멤버 spawn 의 가치(다각 검증)가 0 으로 떨어진다. Lead 의 충돌·누락 탐지 단계가 멀티 에이전트 오케스트레이션의 핵심 부가가치이며, 누락 시 단일 멤버 답변과 동일한 수준의 보고가 된다.
