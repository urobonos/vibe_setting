...
- **스킬 생성·수정·최적화 — skill-creator 강제 진입점 (필수):** `.claude/skills/{skill}/` 하위 모든 파일 생성·수정·최적화 = `skill-creator` 스킬 경유 필수. `skill-edit-guard.sh` PreToolUse hook 가 Edit/Write 시도를 exit 2 차단, 락 파일 (`/tmp/claude_skill_creator_active.lock`) 존재 시에만 우회. **진입 절차:** (1) `/skill-creator` 또는 "스킬 만들기/수정/개선/최적화" 트리거 → (2) `touch /tmp/claude_skill_creator_active.lock` → (3) Edit/Write → (4) 종료 시 `rm` 락. 세션 종료 시 `gate-init.sh` 가 잔여 락 자동 정리 (worktree-enforce.sh exemption #4 `/tmp/claude_*` 자동 매칭 — 2026-05-26 옵션 A 적용).
