...
- **브랜치 워크플로우 — Trunk-Based + Short-lived Feature Branch (필수):** 세션 시작 시 현재 브랜치(`production` / `staging` / `develop` / `main` / `master`)에서 즉시 `feature/{source-branch}_{작업명}` 분기를 생성한다. `{source-branch}` 는 분기 직전 `git rev-parse --abbrev-ref HEAD` 결과 — 분기명 자체에 머지 타깃이 명시됨. 작업명은 kebab-case 영문(기존 `~/.claude/docs/{product}/tasks/{YYYYMMDD}/{작업명}/` 폴더명 규약 재사용). 사용자 명시 머지 승인(`머지` / `merge` / `완료`) 시 분기명에서 추출한 source 브랜치로 `--no-ff` 머지 + 분기 삭제. push 는 별도 승인. **면제 영역:** `~/.claude/docs/{product}/` (산출물) / `projects/.../memory/` (메모리) / `.skill-creator-active.lock` (락 파일). **Why:** 단일 세션 = 분기 수명 보장으로 trunk 안정성 + 분기 단위 revert 가역성 확보. 분기명 자체에 source 가 있어 동일 repo 의 production/staging/develop 다중 환경을 명확히 구분 (env CWD-매핑 방식 폐기 사유). `branch-enforce.sh` PreToolUse hook 이 protected 브랜치 위 직접 변경(Edit/Write/MultiEdit + git commit/merge/rebase/push) 을 exit 2 차단. 산출물 SSOT: `~/.claude/docs/claude-harness/output/branch-workflow/2026-04-30-branch-workflow-design.md`.
