...
- **묶음 승인 Fast-Track (Gate 0→2):** 사용자가 묶음 승인 키워드를 입력하면 `gate-approve.sh` 가 Gate 를 0(또는 1) 에서 곧장 2 로 점프시킨다. analyze.md + plan.md 를 한 응답에 묶어 보고한 뒤 한 번에 승인받는 패턴 지원. 매칭 키워드는 한국어("분석/계획 진행", "둘다 ok", "한번에 승인", "묶어서 진행", "통째로", "모두/전체 진행/승인"), 영어("all ok", "both approve", "all proceed" 등). 단계 문서 검증(`task-docs` 분기)은 0→2 점프에도 동일 강제되며, §3 Checkpoint 5조건은 본 fast-track 으로 우회되지 않는다 (`dangerous-ops-guard` 분리 적용). 상세는 `workflow-enforcer` 스킬 §1 "Gate 0→2 묶음 승인 Fast-Track" 참조. **Claude 측 활용 룰:** M/L 코드 작업에서 분석과 계획이 단일 사이클로 압축 가능하다고 판단되면 analyze.md + plan.md 를 한 응답에 묶어 보고해 사용자가 fast-track 으로 1회 승인할 수 있게 한다. 단 §3 Checkpoint 발동 가능성·아키텍처 결정·트레이드오프가 분석 단계에서 도출될 가능성이 큰 작업은 단계별 보고를 유지한다 (분석 결과에 따라 계획이 크게 달라질 여지가 있는 경우 묶음 보고는 부적합).
...
- **Co-Authored-By 라인 금지 (필수):** git commit 메시지에 `Co-Authored-By: Claude ...` 라인을 포함하지 않는다. **Why:** 사용자(jypark) 단독 author 유지가 정책. **How:** 시스템 기본 commit 템플릿이 Co-Authored-By 추가를 안내해도 무시. `dangerous-ops-guard.sh` 가 hook 으로 강제 차단(SSOT). 위반 시 commit 실패.
