...
- **작업 문서 산출물:** 작업 진행 중 분석 내용과 Pre-Plan은 프로젝트 `docs/` 디렉토리에 파일로 작성한다.
  - **분석/리서치 내용:** `docs/research.md` — 코드 분석, 영향 범위 조사, 기술 조사 등 읽기 전용 탐색 결과를 기록한다.
  - **Pre-Plan:** `docs/plan.md` — 작업 계획, 수정 대상, 예상 영향 범위, Agent Flow Plan을 기록한다.
  - 새 작업 시작 시 기존 내용을 **덮어쓰기** 한다 (이전 작업의 문서는 유지하지 않는다).
  - 사용자에게 보고하는 동시에 해당 파일에도 동일 내용을 기록한다.
