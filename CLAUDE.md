...
- **작업 산출물 경로 (글로벌 통합):** 모든 산출물(`tasks`, `output`, `specs`)은 **`~/.claude/docs/{product}/`** 아래 생성. 프로젝트 레포 내부에 더 이상 생성하지 않는다.
  ...
  - **`output/` 카테고리 (필수):** 신규 산출물은 7 카테고리 중 하나에 배치. 평면 `output/{제목}/` 직접 배치 금지.
    ...
    - **공유용 단일 통합 문서 (필수):** `output/report/.../{share,proposal,sharing}*.md` = 7메타 작성 정보 박스 + 필수 12섹션. 7메타·12섹션 정의·강제 SSOT = `hooks/output-report-share-guard.sh` (PreToolUse exit 2 차단 시 stderr 가 7메타+12섹션 전량 출력).
  ...
  - **`indexing/{product}.md` 전역 문서 인덱스 (필수, 2026-05-29~):** `~/.claude/docs/{product}/` 하위 **전 영역(output / tasks / specs / working 등) 모든 .md 의 추적 인덱스** — `~/.claude/docs/indexing/{product}.md` 한 폴더에 **product별 1파일** 집결 (영역 / 타이틀(파일 내 첫 `# ` 헤더) / 경로 / 수정일 표). `doc-index-maintain.sh` PostToolUse hook 이 docs/{product}/ 하위 .md 의 Edit/Write 시 해당 `indexing/{product}.md` 를 **자동 재생성** (indexing·references 자기 제외 = 무한루프·docset 바이너리 차단, product 별 mkdir lock = race 보호, 비차단 exit 0). **강제점 1개(hook) — docs 에 .md 를 쓰는 모든 스킬·커맨드·훅이 자동 인덱싱**되므로 개별 본문에 index 갱신 지시를 박지 않는다 (드리프트 0, §4.4 장기 관점). **직접 편집 금지** (다음 docs write 시 자동 재생성에 덮어쓰여짐 — 파일 삭제도 다음 재생성 때 자동 반영). 용도 = `/분석`·`/계획` 등 워크플로우의 "참조 범위 전수 조사" 진입점 — 수백 문서를 리터럴 전체 read 하지 않고 **index 전수 스캔 → task 관련 항목만 선택 정독** (토큰 폭발 회피). **한계:** Edit/Write/MultiEdit/NotebookEdit 트리거만 인덱싱 — Bash heredoc·리다이렉트로 직접 쓴 문서는 미반영 (worktree-enforce 가 Bash mutation 차단으로 실사례 거의 없음, 발생 시 임의 docs write 1회로 재생성). SSOT: `hooks/doc-index-maintain.sh` + settings.json PostToolUse 등록 + 본 단락.
...
- **브랜치·worktree·push·머지 통합 정책 (필수, 2026-05-20 재정의):** 단일 SSOT = `hooks/worktree-enforce.sh` + `hooks/worktree-prompt-detect.sh` + `commands/{자동진행,feature-create,feature-merge}.md` + `hooks/branch-enforce.sh` (잔존 영역) + `skills/git-push/SKILL.md`.
  ...
  - **(c) Functional exemption 11건 (`worktree-enforce.sh` SSOT):**
...
- **스킬 생성·수정·최적화 — skill-creator 강제 진입점 (필수):** `.claude/skills/{skill}/` 하위 모든 파일 생성·수정·최적화 = `skill-creator` 스킬 경유 필수. 진입·락 우회·종료 정리 절차 SSOT = `skill-edit-guard.sh` PreToolUse hook (락 파일 `/tmp/claude_skill_creator_active.lock` 미보유 시 Edit/Write exit 2 차단 → stderr 가 락 절차 전량 출력) + 세션 종료 `gate-init.sh` 잔여 락 자동 정리 (worktree-enforce exemption #4 `/tmp/claude_*`).
...
**자동화 분류 카운트:** A = 25 (api-spec-audit · api-team · debate · dev-team · orchestration · report · security-audit · task-docs · working-done · 자동진행 · 작업저장 · 작업로드 · 분석 · 타당성 · 계획 · 실행 · 검증 · 리뷰 · 회고 · 토론 · 조사 · 병렬 · 제안 + mirror-be-claude verify·sync-from-be + sns-oauth verify) / B = 6 (debug-skill · mysql8 · php8 · skill-validator · 프로세스 + sns-oauth add·debug) / C = 11 (aws · bitbucket-cli · feature-create · feature-merge · git-push · notion-cli · skill-creator · workflow-enforcer · 배포 · prod-debug + mirror-be-claude sync-from-global). **A 그룹만 `/loop` · `/schedule` 결합 권장** (SSOT = `output/guide/2026-05-13-loop-schedule-combination/`).
