...
#   - csv 비파싱: 변동성 높은 간트 내용을 hook 이 읽지 않음 → 포맷·내용 변경에 무관.
...
GANTT="$HOME/.claude/docs/참조문서/간트/jyp_gant.csv"
...
이 작업이 사용자(JY Park) 담당 작업인지 간트차트로 대조하세요:
  SSOT: ~/.claude/docs/참조문서/간트/jyp_gant.csv (담당자: JY Park=사용자 / ahn=타 담당자)
판정 — 작업의 WBS ID(MOD-XX-XXX) 또는 작업명/모듈을 csv 에서 조회:
  · 담당자 JY Park       → 진행
