...
#   → 사용자에게 `/hongcafe:mirror-be-claude verify` 명시 호출 유도.
#
# Why:
...
if [ -f "$BE_MD" ] && [ -f "$MIRROR_MD" ]; then
  ...
  if [ "$diff_sec" -gt 60 ]; then
    ...
    echo "[mirror-sanity] CLAUDE.md mtime drift: $diff_sec 초 (newer=$newer) — \`/hongcafe:mirror-be-claude verify\` 권장" >&2
    WARNINGS=$((WARNINGS + 1))
  fi
fi
...
#     file count 완전 일치 검증 = `/hongcafe:mirror-be-claude verify` (sha256 + count) 위임
# ─────────────────────────────────────────────────────────
if [ -d "$GLOBAL_DIR" ] && [ -d "$BE_DIR" ] && [ -d "$DOCS_DIR" ]; then
  ...
  if [ "$((max_t - min_t))" -gt 300 ]; then
    ...
    echo "[mirror-sanity] api-docs top-dir mtime drift: $((max_t - min_t))초 (newer=$newer) — \`/hongcafe:mirror-be-claude verify\` 권장" >&2
    WARNINGS=$((WARNINGS + 1))
  else
    ...
    if [ -n "$sample_file" ]; then
      ...
      if [ "$b_t" -gt 0 ] && [ "$g_t" -gt 0 ] && [ "$((b_t - g_t))" -gt 300 ]; then
        echo "[mirror-sanity] api-docs sample mtime drift: $rel (be > global by $((b_t - g_t))초) — \`/hongcafe:mirror-be-claude verify\` 권장" >&2
        WARNINGS=$((WARNINGS + 1))
      fi
    fi
  fi
