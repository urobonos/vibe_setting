...
**Why:** 산문 단언 + orphan 해시 = 독자가 git log 로 추적 불가 (audit "문서화·추적성" / "정직성" 감점 핵심). hook 강제 불가(자유텍스트)라 /회고 단계 체크로 둔다 (§4.4 — substance 검증 못 하는 건 checklist, 신규 blocker hook 비추천). 정착 시점 예방 = `/feature-merge` §4.
...
**책임 분담선 (audit M14 명문 2026-05-20):** 본 슬래시 책임 = history.md + summary.md 기록 (Persistence §4.1). push·머지·정착 영역 = `/배포` 책임 (별 SSOT). 본 슬래시 = read-only or `~/.claude/docs/` mutation 만, 외부 git 명령 0.
