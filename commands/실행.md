...
| ③ §실행 섹션 채움 | 실행 요약 / 변경 내역 / Before/After 대조 / 테스트 결과 / 잔여 이슈 / 롤백 | Self-Critique ≥ 20 채움 (CLAUDE.md §4.3 "checklist-count-check.sh 임계" SSOT) |
...
> **터미널 제목 설정 (① 직후):** working/ 문서 식별로 작업명이 확정되면 PowerShell 도구로 `$Host.UI.RawUI.WindowTitle = "#{작업명}"` 실행 (예: `#auth-refactor`). 방식·전제·OS·실패 처리 = `commands/작업시작.md` §"터미널 제목 설정 (SSOT)".
