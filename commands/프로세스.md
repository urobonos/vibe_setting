...
| 모드 | 동작 | 비가역성 |
|------|------|---------|
| 기본 (`/프로세스`) | 분류 + 권고만 (read-only) | 없음 |
| `cleanup` (`/프로세스 cleanup`) | REGISTRY orphan entry 제거 + lock orphan 정리 | §3 매칭 — 사용자 명시 인자가 승인 신호 |
| `kill` (`/프로세스 kill`) | 좀비 PID 종료 명령 안내 (사용자 직접 `! ` prefix 실행) | Claude 자동 X — 안내만 |
...
- /프로세스 cleanup → REGISTRY/lock orphan 정리
- /프로세스 kill   → 좀비 PID 종료 명령 안내
```
...
```
/프로세스           ← 분류 + 권고 (read-only)
/프로세스 cleanup   ← REGISTRY orphan + lock orphan 정리
/프로세스 kill      ← 좀비 PID 종료 명령 안내 (사용자 직접)
```
