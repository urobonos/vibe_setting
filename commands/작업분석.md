---
description: 작업 메타분석 — 계획까지 완료된(Status: Plan Complete) 여러 작업을 가로질러 스캔 → 의존·worktree·파일충돌 기준으로 동시진행 그룹 + 우선진행 순위 도출 → working/ 에 메타분석 문서 신규 생성 + 기존 계획 문서에 권고 블록 append. 자립형 커맨드 (본체 스킬 없음). `/분석`(단일 작업)과 구분되는 다작업 메타분석 진입점. 2026-06-09 신설
...
> **[용도 한정]** *여러 작업 간* 동시진행·우선순위 메타분석 전용. 단일 작업 1개의 분석은 `/분석`, 잔존 Partial 작업 로드는 `/작업로드`, 일괄 실행은 `/자동진행` 이 담당한다. 본 커맨드는 그 사이 — **"무엇을 같이/먼저 할지" 계획 수립 진입점**이다.
...
> 출력 = **동시진행 그룹표**(어떤 작업을 같은 응답에서 `/병렬`로 돌릴 수 있는가) + **우선순위 매트릭스**(어떤 그룹·작업을 먼저). 각 판정엔 **근거 1줄**(어느 파일 중복/어느 의존)을 반드시 붙인다 — 근거 없는 "병렬 가능"은 충돌 사고의 원인.
...
분석 대상이 된 각 계획 문서에 권고 블록을 **append** 한다. **전면 재작성 금지** — `doc-template-guard.sh` / `output-naming-check.sh` / `reference-location-check.sh` 양식·메타가 깨지지 않게 기존 본문은 보존하고 아래 블록만 덧붙인다.
...
| 태스크 | 직·병렬 | 방법 |
|--------|---------|-----|
| Phase 1 (작업 N개 메타데이터 추출) | **조건부 병렬** | 작업 ≥ 4 시 작업별 `Explore` Agent 동시 spawn (문서 독립, race 없음). `/병렬 /작업분석` |
...
- 본 커맨드는 **분석·권고 도구** — 권고된 동시진행/우선순위의 **실제 실행(코드 변경·worktree)은 범위 밖**. 후속 `/자동진행` · `/병렬 /자동진행` · `/실행` 로 분리한다.
...
| 슬래시 | 대상 | 범위 |
|--------|------|------|
| `/분석` | **단일** 작업 1개 | working/ 단일 문서 §분석 채움 |
| **`/작업분석`** | **여러** 작업 (Plan Complete) | 작업 간 동시진행 그룹 + 우선순위 메타분석 → working/ 신규 + 계획문서 권고 append |
| `/작업로드` | 잔존 Partial 작업 | working/ Status: Partial 로드 (read-only) |
| `/자동진행` | 잔여 액션·인자 작업 | 묶음 승인 일괄 **실행** (mutation) |
...
```
/작업분석            → 동시진행 그룹 G1{A,B} / G2{C} + 우선순위 도출
   ↓
/병렬 /자동진행 G1   → G1 의 A·B 를 동시 spawn 실행 (충돌 없음 확인됨)
   ↓
/자동진행 C          → 후속 그룹 순차 실행
```
...
| 인자 | 동작 |
|------|------|
| `/작업분석` | 현재 cwd product 의 Plan Complete 작업 메타분석 |
| `/작업분석 all` | 전 product Plan Complete 메타분석 |
| `/작업분석 hongcafe_global_backend` | 특정 product Plan Complete 메타분석 |
| `/작업분석 auth-refactor commerce-price` | 명시 작업들만 메타분석 |

### 호출 예

```
/작업분석                                  ← 본 product 계획완료 작업 동시진행/우선순위 분석
/작업분석 all                              ← 전체 product 가로질러 분석
/작업분석 hongcafe_global_backend          ← be 계획완료 작업만
```
...
|--------|------|
| 단일 작업 1개 분석 | `/분석` |
| 잔존 Partial 작업 로드 | `/작업로드` |
| 작업 실제 실행·구현 | `/자동진행` / `/실행` |
| 의견 갈림·우열 역전 결정 | `/토론` (16 Agent) |

## SSOT

| SSOT | 역할 |
|------|------|
| `~/.claude/commands/작업분석.md` (본 파일) | 자립형 진입점 — 다작업 메타분석 5-Phase 골격 |
| `~/.claude/CLAUDE.md` §5.1 inventory 표 | 커맨드 등록 (자동화 B) |
| `~/.claude/hooks/lib/product-resolver.sh` | cwd → product 산출 (Phase 0 필터) |
| `~/.claude/commands/계획.md` §"step 파일 양식" / Status: Plan Complete | 스캔 대상 마커·WBS 구조 SSOT |
| `~/.claude/commands/자동진행.md` · `~/.claude/commands/병렬.md` | 분석 결과 후속 실행 진입점 |
