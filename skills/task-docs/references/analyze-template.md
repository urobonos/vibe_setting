...
## 6.7. 참조 출처 (Reference Location)

> **무조건 필수 (2026-06-02~)** — 분석 내용의 **출처 위치**를 기록한다 (기획서 = 페이지, docs = 파일:줄). 위 6 "타당성 검토"([Source: §id] 공식 기술표준) 와 **별개**의 내용 provenance. 강제: `doc-unified-check.sh V2` (exit 2 — `## 참조 출처` 섹션 + `[참조: ...]` ≥ 1건). 6.5 "참조 문서 검토 결과" 의 기획안 슬라이드 매핑은 본 표의 한 특수 케이스다.
...
> CLAUDE.md §4.1 "장기 관점 분석·계획·실행" 룰 강제. 누락 시 `doc-unified-check.sh` V1 (§헤더 검증) 차단.
