...
## 실행 계획
- **Team 3 구성:** {Worker Lead + 멤버 목록 — 등급별 구성 SSOT = `orchestration` §1.2 "Task Sizing"}
- **구현 순서:** {레이어별/기능별 순서}
...
- [ ] 분석 대상 파일/모듈 목록 확정
- [ ] 분석 제외 범위 명시
- [ ] 상위/하위 의존 모듈 + 관련 BC(Bounded Context) 식별
- [ ] 영향받는 외부 시스템 식별

### 코드 품질
- [ ] 중복 코드 패턴 — 이미 있는 함수/lib 재사용 가능 여부
- [ ] DI 원칙 준수 여부
- [ ] 에러 핸들링 일관성
- [ ] 매직 넘버/하드코딩 상수 존재
- [ ] 미사용 코드/import 존재 여부
- [ ] **타입 선언 적용 여부** + TODO/FIXME/HACK 잔존 (타입계약 결손 = 라이브 500 유발 클래스 / 주석 우회 = §4.1 위반 artifact)

### 보안
- [ ] SQL Injection / XSS 취약점
- [ ] CSRF 방어 적용 여부
- [ ] 인증·인가 누락 엔드포인트 (권한 상승 가능성 포함)
- [ ] 민감 정보 하드코딩
- [ ] 입력값 검증 누락
...
### 설계 검증
- [ ] 기존 아키텍처 패턴 일관성 + BC 경계 침범 없음
- [ ] 하위 호환성 · 인터페이스 계약 명확성 + **신규 의존성 최소화** (재사용 먼저 — CLAUDE.md §0-2)
- [ ] 에러 처리 전략 정의

### 테스트 계획
- [ ] 단위/통합 테스트 작성 대상 식별
- [ ] 기존 테스트 영향 범위 + 엣지 케이스 목록
...
- [ ] 타당성 검토 공식 근거 제시 완료
- [ ] **변경 영향 기록 작성** + 모니터링 지표 추가 필요 여부 (V3 는 경고뿐이라 차단 게이트가 없다 — 여기가 유일한 확인점)
- [ ] 장기 영향 / 재발 방지 / SSOT 일관성 3섹션 작성

## Self-Critique 체크리스트 (10)

### 보안
- [ ] 민감 정보 노출 없음 (로그, 응답, 에러)
- [ ] 인증/인가 누락 없음
- [ ] 입력값 검증 + SQL Injection / XSS 방어 확인
...
### 코드 품질
- [ ] 컨벤션 · 네이밍 · 하드코딩 · 불필요 코드 정리

### 테스트 커버리지
- [ ] 신규 코드 단위 테스트 작성 + 전체 스위트 통과

### 이전 단계 검증
- [ ] 분석·계획 체크리스트 전체 체크(`[x]`) 완료, 미체크는 잔여 이슈로 기록

> **호출부 전수는 여기서 체크로 끝내지 않는다** — 시그니처·반환 타입·저장 형식을 바꿨으면 `git grep` 으로 형제 호출부를 실제로 뽑아 §변경 내역에 적는다. 부분 적용은 테스트가 green 인 채로 통과한다.
