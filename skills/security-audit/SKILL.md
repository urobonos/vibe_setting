...
8. **참조 출처 (Reference Location)** — 산출물을 파일로 저장 시 `## 참조 출처` 섹션 + `[참조: ...]` ≥ 1건 필수 — 감사 대상 코드의 파일:줄 위치를 기재한다. 강제: `reference-location-check.sh` (output/ exit 2). 형식 예: `[참조: app/Filters/AuthFilter.php:88]`. 이는 6 "타당성 검토" 의 프레임워크 근거(`[Source: owasp-top10 §A01-2021]`)와 **별개**의 내용 provenance — 어디서 그 코드를 봤는가를 추적한다.
