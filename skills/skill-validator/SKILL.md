...
- [ ] 3개 에이전트(Agent-A 경로, Agent-B 구조, Agent-C 정합성)가 **전부** 실행되었는가 (일부 누락 금지)
- [ ] 각 에이전트의 반환 결과에 모든 검사 항목(A: 4개, B: 6개, C: 4개)의 PASS/FAIL 판정이 포함되어 있는가
- [ ] FAIL 항목에 심각도(Critical/High/Medium/Low)가 올바르게 분류되어 있는가
- [ ] 통합 QA 리포트가 §4의 출력 형식을 정확히 따르는가 (종합 결과 + FAIL 상세 + 통계)
